...
static inline intptr_t Aliszof() {
...
struct PluginUtil {
...
#ifndef SSZ_CORE
    // Find the setSSZFunc method and modify it:
    void setSSZFunc() {
#ifdef _WIN32
        // Windows version
...
#else
        // Linux version - use a cast to handle the __stdcall difference
        sszrefnewfunc = reinterpret_cast<void* (*)(intptr_t)>(psf->newfunc);
        sszrefdeletefunc = reinterpret_cast<void (*)(void*)>(psf->deletefunc);
#endif
    }
    
    // Add this to the PluginUtil class declaration
#ifndef _WIN32
    std::string refToStr(Reference ref);
#endif
#endif
    ...
    STAMEM std::WSTR refToWstr(const Reference& strref) {
        std::WSTR wstr;
        if(!strref.null()){
            wstr.append((WCHR*)strref.atpos(), strref.len()/sizeof(WCHR));
        }
        return wstr;
    }
    ...
    static std::string refToAstr(UINT CodePage, const Reference& strref) {
        ...
        if(!strref.null()){
            astr.resize(
                WideCharToMultiByte(CodePage, 0, (wchar_t*)strref.atpos(), (int)strref.len()/sizeof(char16_t), nullptr, 0, nullptr, nullptr));
            WideCharToMultiByte(CodePage, 0, (wchar_t*)strref.atpos(), strref.len()/sizeof(char16_t), (char*)astr.data(), astr.size(), nullptr, nullptr);
        }
        return astr;
    }
    ...
    static void astrToRef(
        ...
        strref.refnew(
            MultiByteToWideChar(
                CodePage, 0, astr.data(), astr.size(), nullptr, 0), sizeof(char16_t));
        if(!strref.null()){
            MultiByteToWideChar(
                CodePage, 0, astr.data(), astr.size(), (wchar_t*)strref.atpos(), strref.len()/sizeof(char16_t));
        }
    }
#else
    ...
    STAMEM std::wstring wToGw(const std::WSTR& wstr) {
        ...
        for(size_t i = 0; i < wstr.size(); i++){
            if(wstr[i] >> 10 == 0x36 && i + 1 < wstr.size() && wstr[i + 1] >> 10 == 0x37) {
                gwstr += wchar_t(((wstr[i]&0x3ff) << 10 | (wstr[i + 1]&0x3ff)) + 0x10000);
            }else{
